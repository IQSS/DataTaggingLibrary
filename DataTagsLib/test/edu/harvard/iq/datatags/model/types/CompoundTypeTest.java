...
public class CompoundTypeTest {
	...
	@Test
	public void testIntersectWith_simple() {
		AtomicType simple_t1 = new AtomicType("st1", null);
		AtomicType simple_t2 = new AtomicType("st2", null);
		...
		/* Empty intersection (same keys: t1, different values) */
		assertEquals(null, a.intersectWith(b));

		/* Shared keys+values intersection */
		CompoundValue expected = ct.createInstance();
		expected.set( v2_t1 ); // as v2_t1.ordinal > v1_t1.ordinal
		expected.set( v1_t2 );
		a.set( v2_t1 );
	...
	@Test
	public void testSubstractKeys_simple() {
		AtomicType simple_t1 = new AtomicType("st1", null);
		AtomicType simple_t2 = new AtomicType("st2", null);
		...
		/* Empty intersection (same keys: t1, different values) */
		assertEquals(null, a.substractKeys(b));

		/* Shared keys+values intersection */
		CompoundValue expected = ct.createInstance();
		expected.set( v1_t2 );

		assertEquals( expected, b.substractKeys(a) );
